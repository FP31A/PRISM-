# -*- coding: utf-8 -*-
"""
PRISM Step 9 Continuation — 9.2, 9.3, 9.4 + Figures
Assumes 9.1 completed and models/prism_best_model.pkl exists.

This script strictly retains the v3 maximum rigor:
  - Regenerates fold-level predictions (no Optuna, strict 100% fold fit via saved best_iters).
  - Runs full nested-CV ablation tuning (re-optimizing every variant properly).
  - Paired bootstrap testing on residuals.
  - Generates publication-ready figures (Parity, Ablation Bar, Learning Curve).
"""
...
TARGET           = "Ea_eV"
SCREEN_THRESHOLD = 4.3    # 2.0 eV (viable reaction threshold)
N_OPTUNA_TRIALS  = 100
N_INNER_FOLDS    = 3
...
# ---------------------------------------------------------------------------
# Optuna Objectives (Rigorous nested-CV versions)
# ---------------------------------------------------------------------------
...
def make_rf_objective(X_train, y_train, cv, seed):
    def objective(trial):
        ...
        scores =[]
        for tr, va in cv:
            m = RandomForestRegressor(**params, n_jobs=N_JOBS, random_state=seed)
        ...
        for fold_i, (tr, va) in enumerate(cv):
            ...
            m = xgb.XGBRegressor(
                **params, early_stopping_rounds=50,
                eval_metric="mae", n_jobs=N_JOBS, random_state=seed, verbosity=0
            )
            m.fit(X_tr, y_tr, eval_set=[(X_es, y_es)], verbose=False)
            trial.set_user_attr("best_iters",
                                trial.user_attrs.get("best_iters",[]) + [m.best_iteration])
            scores.append(mean_absolute_error(y_train[va], m.predict(X_train[va])))
        return np.mean(scores)
    return objective
...
def make_lgbm_objective(X_train, y_train, cv, seed):
    def objective(trial):
        ...
        for fold_i, (tr, va) in enumerate(cv):
            ...
            m = lgb.LGBMRegressor(**params, n_jobs=N_JOBS, random_state=seed, verbose=-1)
            m.fit(X_tr, y_tr, eval_set=[(X_es, y_es)], callbacks=callbacks)
            trial.set_user_attr("best_iters",
                                trial.user_attrs.get("best_iters", []) +[m.best_iteration_])
            scores.append(mean_absolute_error(y_train[va], m.predict(X_train[va])))
        return np.mean(scores)
    return objective
...
def build_model(name, params, n_estimators_override=None, seed=42):
    if name == "Ridge":
        return Pipeline([("sc", StandardScaler()), ("m", Ridge(alpha=params["alpha"]))])
        ...
        return xgb.XGBRegressor(
            n_estimators=n_est, learning_rate=params["lr"],
            max_depth=params["max_depth"], subsample=params["subsample"],
            colsample_bytree=params["colsample"], reg_lambda=params["lambda"],
            eval_metric="mae", n_jobs=N_JOBS, random_state=seed, verbosity=0
        )
    elif name == "LightGBM":
        n_est = n_estimators_override or 2000
        return lgb.LGBMRegressor(
            n_estimators=n_est, learning_rate=params["lr"],
            num_leaves=params["num_leaves"], subsample=params["subsample"],
            colsample_bytree=params["colsample"], reg_lambda=params["lambda"],
...
def fit_model_clean(model, X_train, y_train):
    """Rigorous 100% outer fold fit without early stopping logic leakage."""
...
def tune_and_fit(name, X_train, X_test, y_train, y_test,
    ...
    """Used for Ablation Study (Rigorous re-optimization)"""
    if name == "BEP":
        if "dE_xtb" not in feature_cols:
    ...
    model       = build_model(name, best_params, n_estimators_override=best_n_est, seed=seed)
...
# ---------------------------------------------------------------------------
def regenerate_fold_predictions(df, folds, feature_cols, best_name, params, n_est_override):
    """
    Retrain the best model on each fold using the aggregated params
    from 9.1. Returns a DataFrame used for screening and plotting.
    """
    print("\nRegenerating fold-level predictions from saved model configs...")
    ...
    pred_df = pd.DataFrame({
        "rxn_id": df.iloc[all_idx]["rxn_id"].values,
        "y_true": all_true,
        "y_pred": all_pred,
    })
    pred_df["abs_error"] = (pred_df["y_true"] - pred_df["y_pred"]).abs()
    pred_df.to_csv(f"{OUTPUT_DIR}/screening_predictions.csv", index=False)
    print(f"  Saved to {OUTPUT_DIR}/screening_predictions.csv")
    return pred_df


def run_ablation(df, folds, feature_cols, rxn_class_cols, best_name):
    print(f"\n9.2 ABLATION STUDY (model: {best_name}, full Optuna nested-CV)")
    print("=" * 72)

    variants = {
        "A_topo_only": TOPOLOGICAL_FEATURES + rxn_class_cols,
        "B_elec_topo": ELECTRONIC_FEATURES + TOPOLOGICAL_FEATURES + rxn_class_cols,
        "C_geo_topo":  GEOMETRIC_FEATURES + TOPOLOGICAL_FEATURES + rxn_class_cols,
        "D_full":      feature_cols,
    }
    ...
    for vname, vfeats in variants.items():
        feat_idx =[feature_cols.index(f) for f in vfeats if f in feature_cols]
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(folds):
            ...
            metrics, y_pred, bp, bn = tune_and_fit(
                best_name, X_tr, X_te, y_tr, y_te, inner_cv, fold_idx, local_feature_cols
            )
    ...
    abl_pooled_df = pd.DataFrame(abl_pooled_rows)[["Variant", "MAE", "MedAE", "R2", "Recall", "Precision"]
    ]
    print(abl_pooled_df.to_string(index=False, float_format="{:.4f}".format))
    abl_pooled_df.to_csv(f"{OUTPUT_DIR}/ablation_results.csv", index=False)

    # ---- Paired bootstrap ----
    print("\n  Paired bootstrap (1000 resamples, per-reaction residuals):")
    rng  = np.random.RandomState(42)
    d_ae = np.abs(variant_preds["D_full"]["y_true"] - variant_preds["D_full"]["y_pred"])
    n    = len(d_ae)
    
    bootstrap_rows = []
    for vname in["A_topo_only", "B_elec_topo", "C_geo_topo"]:
        if vname not in variant_preds: continue
        v_ae  = np.abs(variant_preds[vname]["y_true"] - variant_preds[vname]["y_pred"])
        diffs = np.zeros(1000)
        for i in range(1000):
        ...
        print(f"    D < {vname}: Delta MAE={np.mean(diffs):.4f} eV  "
              f"95%CI=[{ci_lo:.4f}, {ci_hi:.4f}]  p={p_val:.4f}")
        bootstrap_rows.append({
            "comparison": f"D < {vname}",
            "delta_MAE": np.mean(diffs),
            "CI_lo": ci_lo, "CI_hi": ci_hi,
            "p_value": p_val
        })
    pd.DataFrame(bootstrap_rows).to_csv(f"{OUTPUT_DIR}/ablation_bootstrap.csv", index=False)

    return abl_pooled_df, variant_preds


def run_screening_analysis(pred_df):
    ...
    for k in["Recall", "Precision", "F1", "MCC", "MAE", "MedAE", "R2"]:
        ...
        print(f"    {k:12s} = {m[k]:.4f}{target}")

    h3 = (not np.isnan(m.get("Recall", np.nan))
           and not np.isnan(m.get("Precision", np.nan))
           and m["Recall"] >= 0.90
           and m["Precision"] >= 0.75)
    print(f"\n  H3 checkpoint: {'PASSED' if h3 else 'FAILED'}")

    pd.DataFrame([m]).to_csv(f"{OUTPUT_DIR}/screening_final.csv", index=False)
    return m


def run_learning_curve(df, folds, feature_cols, best_name, params, n_est_override):
...
# Figure Generation
# ---------------------------------------------------------------------------
def plot_parity(pred_df, output_path):
    """Internal parity plot — Figure 4 left panel."""
    fig, ax = plt.subplots(figsize=(6, 6))
    y_true = pred_df["y_true"].values
    y_pred = pred_df["y_pred"].values

    ax.scatter(y_true, y_pred, alpha=0.3, s=8, color="steelblue", edgecolor="none")
    lo, hi = min(y_true.min(), y_pred.min()) - 0.2, max(y_true.max(), y_pred.max()) + 0.2
    ax.plot([lo, hi], [lo, hi], "k--", linewidth=0.8, label="y = x")
    ax.set_xlim(lo, hi)
    ax.set_ylim(lo, hi)
    ax.set_xlabel("DFT $\\Delta E^{\\ddagger}$ (eV)", fontsize=12)
    ax.set_ylabel("Predicted $\\Delta E^{\\ddagger}$ (eV)", fontsize=12)
    ax.set_title("Transition1x (Internal, Scaffold CV)", fontsize=12)
    ax.set_aspect("equal")
...
def plot_ablation(abl_df, output_path):
    """Ablation bar chart."""
    fig, ax = plt.subplots(figsize=(7, 4))
    variants = abl_df["Variant"].values
    maes     = abl_df["MAE"].values
    colors   =["#d62728", "#ff7f0e", "#2ca02c", "#1f77b4"]

    # If some variants skipped (e.g. BEP without dE_xtb), trim colors to match length
    colors = colors[:len(variants)]

    bars = ax.barh(variants, maes, color=colors, edgecolor="black", linewidth=0.5)
    ax.set_xlabel("MAE (eV)", fontsize=12)
    ax.set_title("Ablation Study — Feature Stream Contributions", fontsize=12)
    ax.invert_yaxis()

    for bar, mae in zip(bars, maes):
        ax.text(bar.get_width() + 0.005, bar.get_y() + bar.get_height()/2,
                f"{mae:.3f}", va="center", fontsize=10)
...
def plot_learning_curve(lc_df, best_name, output_path):
    """Learning curve plot."""
    fig, ax = plt.subplots(figsize=(6, 4))
    ax.errorbar(lc_df["n_train_approx"], lc_df["MAE_mean"], yerr=lc_df["MAE_std"],
...
def main():
    ...
    # ── Regenerate fold predictions ──
    pred_df = regenerate_fold_predictions(
        df, folds, feature_cols, best_name,
        saved_params, saved_n_est
    )

    # ── 9.2 Ablation (full Optuna rigorous) ──
    abl_pooled, variant_preds = run_ablation(
        df, folds, feature_cols, rxn_class_cols, best_name
    )

    # ── 9.3 Screening ──
    screen_metrics = run_screening_analysis(pred_df)
    ...
    # ── Figures ──
    print("\nGenerating figures...")
    plot_parity(pred_df, f"{FIGURES_DIR}/parity_internal.png")
    plot_ablation(abl_pooled, f"{FIGURES_DIR}/ablation_barplot.png")
    plot_learning_curve(lc_df, best_name, f"{FIGURES_DIR}/learning_curve.png")

    # ── Final summary ──
    print("\n" + "=" * 72)
    print("=== Step 9v3 Continuation Complete ===")
    print("=" * 72)
    
    print(f"Screening:")
    print(f"  Recall:      {screen_metrics['Recall']:.4f}  (>= 0.90)")
    print(f"  Precision:   {screen_metrics['Precision']:.4f}  (>= 0.75)")
    h3 = (screen_metrics.get("Recall", 0) >= 0.90
           and screen_metrics.get("Precision", 0) >= 0.75)
    ...
    if len(abl_pooled) == 4:
        ...
        if a_mae > c_mae > b_mae > d_mae:
            ...
            print(f"\n  Hierarchy: A={a_mae:.3f} B={b_mae:.3f} C={c_mae:.3f} D={d_mae:.3f}")

    print(f"\nOutputs generated in {OUTPUT_DIR}/ and {FIGURES_DIR}/")
